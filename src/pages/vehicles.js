...
export function renderVehicles() {
  ...
  const bodyContent = `
    <div class="card mb-6" style="border-left:4px solid var(--c-accent-light);background:var(--bg-elevated)">
      <div class="card-body" style="display:flex;gap:var(--sp-5);align-items:flex-start;flex-wrap:wrap">
        <div style="flex:1;min-width:220px">
          <div style="display:flex;align-items:center;gap:var(--sp-2);margin-bottom:var(--sp-2)">
            ...
            <span style="font-weight:700;font-size:var(--fs-base)">What is Vehicle Registry?</span>
          </div>
          <p style="font-size:var(--fs-sm);color:var(--text-secondary);line-height:1.7;margin:0">
            This is your <strong>digital garage</strong> — the central place to add, view, update, or remove every vehicle your company owns. Keep all your fleet assets organised in one place.
          </p>
        </div>
      ...
      { icon: 'pin', label: 'License Plate', desc: 'Unique ID — Indian format (e.g. GJ-01-AB-1234)' },
      { icon: 'scale', label: 'Max Load Capacity', desc: 'Max weight the vehicle can safely carry (kg)' },
      { icon: 'speed', label: 'Odometer', desc: 'Current mileage reading on the dashboard (km)' },
    ].map(item => `
            <div style="display:flex;gap:var(--sp-2);align-items:flex-start;min-width:180px">
